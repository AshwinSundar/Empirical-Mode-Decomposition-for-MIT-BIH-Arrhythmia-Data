function [aDeviationMatrixM1, aDeviationMatrixM2, aDeviationMatrixM3, aDeviationMatrixM4, aDeviationMatrixM5, aDeviationMatrixM6] = fDeviationMatrix2

% to analyze just a few select arrhythmias - (B, A, F, N , V
iNumFiles = 5; % number of arrhythmia types
sfileName = 'PhysioBank Records.xlsx';
iNumModes = 6; % first 6 modes exist across the select arrhythmias
...
aDeviationMatrixM2 = zeros(iNumFiles); % first 6 modes exist across select arrhythmias
aDeviationMatrixM3 = zeros(iNumFiles);
aDeviationMatrixM4 = zeros(iNumFiles);
aDeviationMatrixM5 = zeros(iNumFiles);
aDeviationMatrixM6 = zeros(iNumFiles);

fprintf('Select range of arrhythmia abbreviations in sheet 6\n'); % prompt for xlsread
