...
for j = 1:length(aMITDBFileNumbers) % go through all the files
    ...
    fprintf('Select range of arrhythmia abbreviations\n'); % prompt for xlsread
    ...
    newdir = strcat('mitdb', num2str(aMITDBFileNumbers(j))); 
    mkdir(newdir); % make a fresh directory to hold the arrhythmia windows
    ...
    for i = 1:length(aArrhythmiaDemarcations) % go through all arrhythmias for each file
        ...
        sArrhythmiaType = char(deblank(aArrhythmiaTypes(i,:))); % deblank removes trailing spaces
        ...
        sWindowFileName = strcat(sMITDBFileName, '_', sMITDBArrhythmiaNumber, '_', sArrhythmiaType); 
