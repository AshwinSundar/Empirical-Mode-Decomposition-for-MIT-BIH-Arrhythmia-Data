...
%
% Cf:  Huang et al, Proc. Roy. Soc. Lond. A 454, 903 (1998)
...
function imf = emd(x)
        ...
        while SD > 0.3   % while the standard deviation is higher than 0.3 (typical value)
            ...
            for i=1:N1-1
                if maxes(i) == maxes(i+1)
                    sprintf('test')
                    sprintf('Duplicate in max array at %d: %d, %d', i, maxes(i), maxes(i+1))
                end
            end
            N2 = length(mins);
            for i=1:N2-1
                if mins(i) == mins(i+1)  
                    sprintf('test')
                    sprintf('Duplicate in min array at %d: %d, %d', i, mins(i), mins(i+1))
                end
            end
    ...
    c2 = clock;
    %t2 = sprintf('%d:%.1f', c2(5), c2(6))

    % elapsed_time = sprintf(' %.2f seconds', ((c2(4) - c1(4)) + (c2(5) - c1(5)) / 60. + (c2(6) - c1(6)) / 3600.) * 60 * 60.)
